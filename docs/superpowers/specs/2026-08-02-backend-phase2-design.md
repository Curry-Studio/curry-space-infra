...
1. **No real backend app image yet.** ECS task definitions, services, and the ALB target group get created, but there is no image pushed to ECR yet — services may sit with zero healthy tasks until one exists. This is intentional, not a bug to fix in this phase. (Updated 2026-08-02: the backend app repo, `curryspacebe`, turned out to already exist with substantial real code — a `beta` branch now has placeholder `worker.ts`/`scheduler.ts` entrypoints alongside the real Express API, see decision 7 below. What's still missing is a built-and-pushed image, not the code.)
...
7. **Port and health-check path match the real `curryspacebe` app, not the architecture doc's placeholder values.** The doc assumed port `8080` and `/health`; the actual Express app defaults to port `3000` (`env.PORT`) and exposes `/healthz`/`/readyz`. Fixed in this spec and the plan wherever they appeared — target group port, security group rules, listener rule path conditions, and the API service's `container_port`. The API's command is `node dist/index.js`, not `dist/main.js` — the app has one entrypoint today, not the doc's three; `worker.ts`/`scheduler.ts` were added as minimal placeholder entrypoints on `curryspacebe`'s `beta` branch specifically so the worker/scheduler services have something real to reference.
...
One target group, `cs-<env>-use1-api-tg`, port 3000, target type `ip` (required for Fargate's `awsvpc` mode), health check on `/healthz` expecting `200`, deregistration delay 30s. Routing: priority 20 returns a fixed 200 for `/healthz` directly at the ALB (so a health check doesn't need to reach a task at all); priority 100 forwards everything else to `api-tg`. Port and health path match `curryspacebe`'s actual app (`env.PORT` defaults to `3000`, health route is `/healthz`/`/readyz`) rather than the architecture doc's `8080`/`/health` placeholder values — the real code wins once it exists.
...
|---|---|---|---|---|
| API | `node dist/index.js` | 3000 | 100% `FARGATE` | 1 task, 0.5 vCPU / 1 GB, no auto scaling |
...
3. Confirm the ALB responds on 443 with a valid certificate; `/healthz` fixed-response rule returns 200 directly from the ALB without touching a target.
4. Confirm `https://beta.curry.space/api/healthz` (through CloudFront) reaches the ALB and gets the same fixed 200 — this validates the edge integration end-to-end even with zero real backend tasks running, since the ALB-level fixed-response rule doesn't need a healthy target.
